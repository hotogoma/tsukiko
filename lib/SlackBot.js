...
class SlackBot extends EventEmitter {
  constructor(token, opts = {}) {
    ...
    this.client.rtm.on(Slack.CLIENT_EVENTS.RTM.AUTHENTICATED, (data) => {
    ...
    this.client.rtm.on(Slack.CLIENT_EVENTS.RTM.RTM_CONNECTION_OPENED, () => this.emit(EVENTS.CONNECTION.OPENED));
    this.client.rtm.on(Slack.RTM_EVENTS.MESSAGE, this._onReceive.bind(this));
  ...
  send(msg) {
    ...
    msg.attachments ? this._sendWeb(msg) : this.client.rtm.send(msg);
  }

  _sendWeb(msg) {
    msg.as_user = true;
    this.client.web.chat.postMessage(msg.channel, undefined, msg);
  }
